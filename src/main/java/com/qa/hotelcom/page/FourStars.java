...
public class FourStars {
	...
	public FourStars(WebDriver driver) {
		this.driver = driver;
		eUtil = new ElementUtil(driver);
		wait = new WebDriverWait(driver, 10);
	}

	By star = By.xpath("//input[@id='f-star-rating-4']");
	...
	public void getPageTitle(String expectedTitle) {
		...
		System.out.println("FourStarhotelsPage Title is : \n" + title);
		try {
	...
	public void fourStarHotelsandDistance() {
	...
	public void printclickFiveStarHotels() {
		...
		for (int i = 0; i < cityCenterDistance.size(); i++) {
			double mile = Double.parseDouble(cityCenterDistance.get(i).getText()
					.substring(0, 3));
			
			if (mile <= 10.0) {
				List<WebElement> hotelNames = driver.findElements(hotels);
				System.out.println((++a) + "- " + hotelNames.get(i).getText() + 
